...
void sqlite3_connection::encypt(const char* pwd,int len){
	if(!this->db) throw database_error("database is not open");
	if(sqlite3_rekey(db,pwd,len) != SQLITE_OK)
		throw database_error(*this);
}

void sqlite3_connection::decypt(const char* pwd,int len){
	if(!this->db) throw database_error("database is not open");
	if(sqlite3_key(db,pwd,len) != SQLITE_OK)
		throw database_error(*this);
}
